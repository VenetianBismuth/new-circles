...
//STUFF THAT WON'T WORK SO PRETTY MUCH USELESS----------------------------------------
...
int main() {
    ...
    InitAudioDevice();              
    Music start = LoadMusicStream("resources/ph.mp3");
    ...
    Image image11 = LoadImage("resources/purple.png");
    Texture2D purple = LoadTextureFromImage(image11);
    UnloadImage(image11);
    
    Image image12 = LoadImage("resources/koray.png");
    Texture2D koray = LoadTextureFromImage(image12);
    UnloadImage(image12);
    ...
    float lineArr[7] = { 40.0, 45.0, 50.0, 55.0, 60.0, 65.0, 70.0 };
    srand(time(NULL));
    int randomIndexL = rand() % 7;
    float randomLine;
    randomLine = lineArr[randomIndexL];
    //----------------------------------------------------------------------------------------
    float x0, y0, theta, currentX, currentY, parentX, parentY;
    x0 = 600.0;
    y0 = 400.0;
    theta = 0.0;
    ...
    circles.positionX = currentX;
    circles.positionY = currentY;
    ...
    while (!WindowShouldClose()) {
        ...
        timePlayed = GetMusicTimePlayed(start) / GetMusicTimeLength(start);

        if (timePlayed > 1.0f) timePlayed = 1.0f;

        currentX = x0 + randomLine * cos(theta);
        currentY = y0 + randomLine * sin(theta);
        parentX = currentX;
        parentY = currentY;
        theta += randomVelocity;

        currentX += GetFrameTime() * 100;
        currentY += GetFrameTime() * 100;
        ...
        if (addCircles) {
            DrawLine(screenWidth / 2.0, screenHeight / 2.0, currentX, currentY, BLACK);
            DrawCircle(currentX, currentY, randomRadius, randomColor);
            DrawCircle(currentX, currentY, 2, BLACK);
            DrawCircleLines(currentX, currentY, randomRadius, DARKGRAY);

            DrawText(TextFormat("VELOCITY= %f", randomVelocity), screenWidth / 2.0 - 100, 15, 16, GRAY);
            if (randomVelocity > 0.20) {
                DrawTexture(texGojo, GetScreenWidth() / 2 - texGojo.width / 2, 500, RAYWHITE);
            }

            if (&randomColor == &DARKPURPLE) {
                DrawTexture(purple, screenWidth / 2 - purple.width / 2, screenHeight / 2 - bob.height / 2, RAYWHITE);
            }
        }
        ...
        if (buttonClicks == 20) {
            DrawTexture(koray, screenWidth / 2 - koray.width / 2, screenHeight / 2 - koray.height / 2, RAYWHITE);
        }
